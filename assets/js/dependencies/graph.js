;( function() {
    ...
    function drawDetailedInformation ( data, element ) {
      ...
      if ( ( bBox.x + bBox.width / 2 ) > 0 ) {
        ...
        infoContainer = detailedInfo.append( 'g' )
          ...
          .attr(
            'transform',
            'translate(' + 0 + ',' + ( bBox.height + bBox.y + 100) + ')'
          );
        anchor   = 'start';
        position = 'left';
      }
      ...
      infoContainer.data( [ data.value * 100 ] )
        .append( 'text' )
        .text ( '0 %' )
        .attr( 'class', 'pieChart--detail--percentage--'+data.color )
  ...
  function ಠ_ಠ() {
    //getHistorical(function (data){
    //  console.log(data);
    //  drawLineChart(    'lineChart',    data[0] );
    //});
    getRatio(function (ratio){
      for (var i in data.pieChart){
        if (data.pieChart[i].title.toLowerCase() in ratio){
          data.pieChart[i].value = ratio[data.pieChart[i].title.toLowerCase()];
        }
      }
  ...
  function getHistorical(callback){
    $.ajax({
